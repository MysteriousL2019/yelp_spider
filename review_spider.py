...
def scrap_review(url):
    ...
    check_unclaimed = driver.find_elements(By.XPATH,'//span[contains(@class," css-1luukq")]/span[contains(@class," hovercard-trigger__09f24__qn3fP")]/div[contains(@class," css-v3nuob")]/span[contains(@class,"bullet--dark__09f24__iJ3M2 css-1deadz8")]/a[contains(@class,"css-19v1rkv")]')
    if len(check_unclaimed) != 0 and check_unclaimed[0].text == 'Unclaimed':
        print('Unclaimed')
        return []
    ...
    ### I have obtained everything I can, including time, people, where I come from, comments, and ratings
    for page in range(0, total_page_num*10, 10):
        
        if page != 0:
            url = basic_url.format(page)
        driver.get(url)
        # Wait for the page to load, you can increase the waiting time according to the actual situation
